...
- [Development guidelines](#development-guidelines)
    - [General Litecoin-PoS Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Litecoin-PoS Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Litecoin-PoS Core is a multi-threaded application, and deadlocks or other
...
$ valgrind -v --leak-check=full src/litecoin-posd -printtoconsole
...
To profile a running litecoin-posd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep litecoin-posd` -- sleep 60
```
...
Litecoin-PoS Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Litecoin-PoS Core, where everyone uses
...
pay attention to for reviewers of Litecoin-PoS Core code.

General Litecoin-PoS Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Litecoin-PoS Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Litecoin-PoS Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Litecoin-PoS Core so that they can be integrated
...
file descriptors, it will cause problems with Litecoin-PoS's `select()` loop, because
...
compatibility between nodes. This happened in Litecoin-PoS 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Litecoin-PoS's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `litecoin-pos-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `litecoin-pos-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Litecoin-PoS Core, the wallet was always
