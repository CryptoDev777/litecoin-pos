...
`/Applications/Bitcoin-Qt` (on Mac) or `litecoin-posd`/`litecoin-pos-qt` (on
Linux).
...
`contrib/init/litecoin-posd.service`, it has been changed to use
`/var/lib/litecoin-posd` as the data directory instead of
`~bitcoin/.bitcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/litecoin-posd` will
exist has enough space (check using `df -h /var/lib/litecoin-posd`), and
...
The systemd init file (`contrib/init/litecoin-posd.service`) has been changed
to use `/var/lib/litecoin-posd` as the data directory instead of
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart litecoin-posd with the `-deprecatedrpc=generate`
...
- A new `litecoin-pos-wallet` tool is now distributed alongside Bitcoin
...
- #13926 [Tools] litecoin-pos-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like litecoin-posd (MarcoFalke)
...
- #14521 qt, docs: Fix `litecoin-pos-qt -version` output formatting (hebasto)
...
- #15154 Configure: litecoin-pos-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/litecoin-pos-wallet (Sjors)
...
- #12246 Bugfix: Only run litecoin-pos-tx tests when litecoin-pos-tx is enabled (luke-jr)
...
- #14186 litecoin-pos-cli: don't translate command line options (HashUnlimited)
...
- #15354 Add missing `litecoin-pos-wallet` tool manpages (MarcoFalke)
