...
- Adding `dbcache=100` in litecoin-pos.conf
...
litecoin-pos-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/litecoin-pos-cli -stdin walletpassphrase
    mysecretcode
    120
...
- #7550 `8b958ab` Input-from-stdin mode for litecoin-pos-cli (laanwj)
...
- #7613 `3798e5d` Add autocomplete to litecoin-pos-qt's console window (GamerSg)
...
- #8256 `af2421c` BUG: litecoin-pos-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from litecoin-posd (sipa)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for litecoin-posd procs (MarcoFalke)
...
- #7744 `d8e862a` test_framework: detect failure of litecoin-posd startup (laanwj)
