...
This allows running litecoin-posd without having to do any manual configuration.
...
`litecoin-pos.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ...
        # optional bypass litecoin-posd rpc basic auth
...
Restarting litecoin-posd will no longer clear out the list of banned peers; instead
...
- #6600 `ef4945f` Include litecoin-pos-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix litecoin-pos-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of litecoin-pos-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [litecoin-pos-cli] improve error output (Jonas Schnelli)
