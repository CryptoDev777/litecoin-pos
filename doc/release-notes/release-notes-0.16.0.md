...
- litecoin-pos-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- litecoin-pos-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest litecoin-pos.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11970 `b7450cd` Add test coverage for litecoin-pos-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to litecoin-pos-cli to allow RPC password to be read from standard input (jharvell)
...
- #10871 `c9a4aa8` Handle getinfo in litecoin-pos-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop litecoin-posd (OmeGak)
...
- #10529 `331352f` Improve litecoin-posd systemd service file (Flowdalic)
