...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `litecoin-pos.conf` file, not inside included files or from
  ...
  as litecoin-pos.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the litecoin-pos.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
be used in V0.17 by starting litecoin-posd with the '-deprecatedrpc=accounts'
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart litecoin-posd with
...
- When running litecoin-posd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If litecoin-posd
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI litecoinpos:// (krab)
...
- #13528 `b877c39` Move LitecoinPosGUI initializers to class, fix initializer order warning (laanwj)
...
- #13604 `75bea05` Add depends 32-bit arm support for litecoin-pos-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling litecoin-pos-qt (TheCharlatan)
...
- #12443 `07090c5` Move common args to litecoin-pos.conf (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in litecoin-posd systemd service file (Flowdalic)
- #12652 `55f490a` litecoin-pos-cli: Provide a better error message when litecoin-posd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to litecoin-posd in debian package (ghost)
...
- #13603 `453ae5e` litecoin-pos-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix litecoin-pos-cli --version (Empact)
...
- #12877 `2b54155` Use litecoin-posd in Tor documentation (knoxcard)
...
- #13905 `73a09b4` fixed litecoin-pos-cli -help output for help2man (hebasto)
