# PSBT Howto for Litecoin-PoS Core

Since Litecoin-PoS Core 0.17, an RPC interface exists for Partially Signed Litecoin-PoS
...
PSBT is an interchange format for Litecoin-PoS transactions that are not fully signed
...
Overall, the construction of a fully signed Litecoin-PoS transaction goes through the
following steps:
...
- An **Extractor** produces a valid Litecoin-PoS transaction (in network format)
...
#### Multisig with multiple Litecoin-PoS Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Litecoin-PoS Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `litecoin-pos-cli` is used.
